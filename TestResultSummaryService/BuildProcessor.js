    ...
    async execute(task) {
        ...
        if (buildInfo) {
            ...
            if (task.status === "Streaming") {
                ...
                // If all child nodes are done, set current node done
                const testResultsDB = new TestResultsDB();
                const childBuilds = await testResultsDB.getData({ parentId: task._id }).toArray();
                let done = true;
                for (let childBuild of childBuilds) {
                    if (childBuild.status != "Done") {
                        done = false;
                        break;
                    }
                }
